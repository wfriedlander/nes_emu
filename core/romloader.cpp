...
uint32_t crc32(iNesRom& ines)
{
    ...
    for (auto& byte : ines.trainer) {
        crc = (crc32_table[(crc ^ byte) & 0xff] ^ ((crc) >> 8));
    }
    for (auto& byte : ines.prg) {
        crc = (crc32_table[(crc ^ byte) & 0xff] ^ ((crc) >> 8));
    }
    for (auto& byte : ines.chr) {
        crc = (crc32_table[(crc ^ byte) & 0xff] ^ ((crc) >> 8));
    }
    return ~crc;
}
...
RomLoader::RomLoader()
{
    ...
    while (std::getline(file, line)) {
        ...
        std::getline(stream, crc, ',');

        DBRecord desc;

        std::getline(stream, desc.system, ',');
        std::getline(stream, desc.board, ',');
        std::getline(stream, desc.pcb, ',');
        std::getline(stream, desc.chip, ',');
        std::getline(stream, cell, ',');
        desc.mapper = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, cell, ',');
        desc.prg = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, cell, ',');
        desc.chr = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, cell, ',');
        desc.vram = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, cell, ',');
        desc.wram = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, cell, ',');
        desc.sram = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, cell, ',');
        desc.battery = cell.empty() ? 0 : std::stoi(cell);
        std::getline(stream, desc.mirroring, ',');
        std::getline(stream, desc.controller, ',');
        std::getline(stream, desc.conflicts, ',');
        std::getline(stream, desc.submapper, ',');
...
Result<iNesRom> LoadRom(std::string filename)
{
    ...
    st.read(reinterpret_cast<char*>(ines.trainer.data()), 512 * ines.trainer_size);
    ines.prg.resize(ines.prg_size * 1024 * 16);
    st.read(reinterpret_cast<char*>(ines.prg.data()), ines.prg_size * 1024 * 16);
...
Result<Mapper*> RomLoader::Load(std::string filename)
{
    ...
    if (mRoms.count(crc)) {
        DBRecord rec = mRoms[crc];
        cart.prg_size = rec.prg;
        cart.chr_size = rec.chr;
        cart.vram_size = rec.vram;
